  ...
  try {
    ...
    const sites = await db.insert(schema.submittedSites).values([
      ...
      },
      {
        ...
        description: "Kukode is a premium directory platform for web developers. Register accounts, submit your modern web products, upvote creations, write reviews, and build curated collections.",
      ...
      },
      {
        ...
        description: "GeekFolio is a drag-and-drop builder for developer portfolios. Integrates directly with GitHub, dev.to, and Medium to showcase your work automatically.",
    ...
    await db.insert(schema.comments).values([
      {
        ...
        user_id: johnUser.id,
        content: "This Astro template looks incredibly slick! The typography hierarchy is absolutely top-notch."
      },
      {
        site_id: carbonSite.id,
        user_id: janeUser.id,
        content: "Love the minimalist vibe and performance. Astro and Tailwind v4 are a great combination."
      },
      {
        site_id: kukodeSite.id,
        user_id: johnUser.id,
        content: "Exactly the directory modern developers need. Love that it's connected with Turso DB!"
      }
    ]);
